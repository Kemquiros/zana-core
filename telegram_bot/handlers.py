...
ALLOWED_USERS = set(filter(None, os.getenv("ZANA_TELEGRAM_ALLOWED_USERS", "").split(",")))
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        icons = {"stt": "🎤", "tts": "🔊", "llm": "🧠", "vision": "👁️",
                 "kalman": "📐", "armor": "🛡️"}
        for k, v in backends.items():
            lines.append(f"{icons.get(k, '·')} `{k}`: {v}")
        await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN)
    except Exception:
        await update.message.reply_text("⚠️ Gateway offline. Ejecuta `zana start` en tu servidor.")
...
async def cmd_recall(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not records:
            await update.message.reply_text("_Sin registros episódicos aún._", parse_mode=ParseMode.MARKDOWN)
            return
...
async def cmd_reason(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if result.get("swarm_rule"):
            ...
            lines.append(f"🌐 _Swarm contribuyó: {sr.get('name')} ({sr.get('votes')} votos)_")
...
async def cmd_aeon(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not aeons:
        await update.message.reply_text("_No se encontró el registro de Aeons._",
                                        parse_mode=ParseMode.MARKDOWN)
        return
...
async def handle_voice(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        icon = _emotion_icon(emotion)
        reply = f"🎤 _{transcript}_\n\n{icon} {response}" if transcript else f"{icon} {response}"
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("aeon:"):
        ...
        await query.edit_message_text(f"✓ Aeon cambiado a `{aeon_id}`.\n_Usa /sense para continuar._",
                                      parse_mode=ParseMode.MARKDOWN)
