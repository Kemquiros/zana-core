...
class VisionProcessor:
    ...
    def _extract_keyframes(
        ...
        try:
            ...
            logger.info("🎬 [VISION] %d keyframes extracted (fps=%.1f, step=%d)", len(frames), fps, step)
            return frames
    ...
    def _parse_vision_response(self, raw: str) -> tuple[str, VisionFeatures]:
        ...
        try:
            ...
            logger.info("👁️  [VISION] Scene: %s | Entities: %s", desc[:60], features.entities[:3])
            ...
            return raw[:200], VisionFeatures(
                scene_type="mixed", entities=[], emotional_context="neutral",
                text_content="", confidence=0.4,
            )
