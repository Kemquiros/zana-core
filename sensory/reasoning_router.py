...
class _Condition:
    def __init__(self, fact_key: str, operator: str, value: Any, field_path: str | None = None):
    ...
    def matches(self, kb: dict[str, Any]) -> bool:
        ...
        if raw is None:
            return False
        v = raw.get(self.field_path) if self.field_path and isinstance(raw, dict) else raw
        v = _coerce(v)
        cmp = self.value
        match self.operator:
            case "eq":   return v == cmp
            case "neq":  return v != cmp
            case "gt":   return isinstance(v, float) and isinstance(cmp, float) and v > cmp
            case "lt":   return isinstance(v, float) and isinstance(cmp, float) and v < cmp
            case "gte":  return isinstance(v, float) and isinstance(cmp, float) and v >= cmp
            case "lte":  return isinstance(v, float) and isinstance(cmp, float) and v <= cmp
            case "contains":
...
class _Rule:
    def __init__(self, name: str, conditions: list[_Condition],
                 effects: list[str], explanation: str, confidence: float = 0.9):
        self.name = name
...
class _PyReasoningEngine:
    ...
    def infer(self, facts: dict[str, Any], extra_rules: list[dict] | None = None) -> dict:
        ...
        if extra_rules:
            for rd in extra_rules:
                try:
                    conds = [
                        _Condition(c["fact_key"], c.get("operator", "eq"), c["value"],
                                   c.get("field_path"))
        ...
        for rule in rules:
            if all(c.matches(kb) for c in rule.conditions):
                ...
                try:
                    explanation = explanation.format(**{k: round(v, 3) if isinstance(v, float) else v
                                                         for k, v in kb.items()})
                except Exception:
                    pass
        ...
        "rules": [
            {
                ...
                "conditions": [
                    {"fact_key": c.fact_key, "operator": c.operator,
                     "value": c.value, "field_path": c.field_path}
