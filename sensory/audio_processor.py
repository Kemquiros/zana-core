...
class AudioProcessor:
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self._backend = "mock"
                logger.warning("⚠️  [AUDIO] No STT engine found. Install: uv add faster-whisper")

    # ── Public API ───────────────────────────────────────────────────────────

    def transcribe(self, audio_bytes: bytes, mime_type: str = "audio/wav") -> TranscriptionResult:
        """
    ...
    def _transcribe_faster(self, path: str, features: AudioFeatures) -> TranscriptionResult:
        segments, info = self._model.transcribe(path, beam_size=3, language=None)
        transcript = " ".join(s.text.strip() for s in segments).strip()
        avg_conf = float(np.mean([np.exp(s.avg_logprob) for s in
                                  self._model.transcribe(path, beam_size=3)[0]] or [0.0]))
    ...
    def _transcribe_openai(self, path: str, features: AudioFeatures) -> TranscriptionResult:
        import whisper
