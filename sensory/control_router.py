...
def record_shadow_event(event_type: str, payload: dict | None = None) -> None:
    ...
    logger.debug("👁️  [SHADOW] event=%s total=%d", event_type, _shadow_state["events_captured"])
...
async def swarm_init(req: SwarmInitRequest):
    ...
    if _swarm_state["running"]:
        return {"error": "swarm_already_running",
    ...
    logger.info("🐝 [SWARM] Red Queen online — %d warriors, %d generations", req.warriors, req.generations)
    return {
...
async def swarm_status():
    return {
        ...
        "summary": {
            ...
            "active": sum(1 for w in _swarm_state["warriors"] if w["status"] == "active"),
            "legends": sum(1 for w in _swarm_state["warriors"] if w["stage"] == "legend"),
            "avg_fitness": round(
                sum(w["fitness"] for w in _swarm_state["warriors"]) / max(len(_swarm_state["warriors"]), 1),
                3,
            ) if _swarm_state["warriors"] else 0.0,
        },
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            ...
            if resp.status_code != 200:
                return {"found": False, "error": f"registry_returned_{resp.status_code}"}

            rules = resp.json()
            candidates = [
                r for r in rules
                if any(c.get("fact_key") == req.fact_key
