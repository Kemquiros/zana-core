...
from zana.tui.aeon_battle import WORLDS, BattleEngine
from zana.tui.aeon_dna import derive_dna
...
def cmd_worlds():
    ...
    table = Table(
        title="Mundos del Multiverso", show_header=True, header_style="bold magenta"
    )
...
def cmd_enter(
    world_id: Annotated[
        str, typer.Argument(help="ID of the world to enter (e.g. fuego, calculo)")
    ] = "fuego",
):
    ...
    guardian_dna = derive_dna(
        "GUARDIAN", engine.world.native_archetype, birth_seed=12345
    )
    guardian = {
...
def cmd_challenge(
    aeon_name: Annotated[
        str, typer.Argument(help="Name of the summoned Aeon to challenge")
    ],
):
    ...
    if not summon_path.exists():
        console.print(
            f"[error]Aeon '{aeon_name}' not found in summons. Use `zana aeon summon` first.[/error]"
        )
        return
