...
def cmd_shadow_enable() -> None:
    ...
    if pid and _is_running(pid):
        console.print(f"[warning]Shadow Observer already running (PID {pid}).[/warning]")
        return

    # Find shadow_observer entry point
    shadow_src = Path(__file__).parent.parent.parent.parent.parent / "shadow_observer" / "src" / "main.rs"
    shadow_py = Path(__file__).parent.parent.parent.parent.parent / "shadow_observer" / "shadow_daemon.py"
    ...
    try:
        ...
        console.print("[error]Gateway offline. Start the stack first: zana start[/error]")
    ...
    if pid and _is_running(pid):
        ...
        try:
            ...
            table.add_row("Daemon",
                          "[success]✓ Active[/success]" if active else "[muted]— Inactive[/muted]",
                          "via Gateway")
            if active:
                table.add_row("Events captured", str(data.get("events_captured", "?")), "")
                table.add_row("Last event", data.get("last_event", "—"), "")
        except Exception:
            table.add_row("Daemon", "[muted]— Inactive[/muted]", "run: zana shadow enable")

    console.print(Panel(table, title="[header] Shadow Observer Status [/header]",
