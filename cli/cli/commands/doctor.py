...
def _cmd_version(cmd: list[str]) -> str | None:
    try:
        return subprocess.check_output(cmd, stderr=subprocess.DEVNULL, text=True).strip().splitlines()[0]
    except Exception:
        return None
...
def _env_check() -> list[tuple[str, str, str]]:
    ...
    if env_file.exists():
        ...
        for key in required:
            ...
            rows.append((f"  {key}", "[success]✓ Set[/success]" if present else "[error]✗ Missing[/error]", "required"))
        for key in optional:
            val = os.getenv(key, "")
            present = bool(val) or key in content
            rows.append((f"  {key}", "[success]✓ Set[/success]" if present else "[muted]— not set[/muted]", "optional"))
    else:
...
def cmd_doctor() -> None:
    ...
    def _ok(v: str | None, min_label: str = "") -> str:
        ...
        return f"[error]✗ not found[/error]" + (f" (need {min_label})" if min_label else "")
    ...
    if shutil.which("docker"):
        try:
            ...
            env_table.add_row("Docker daemon", "[error]✗ Not running[/error]", "run: sudo systemctl start docker")

    console.print(Panel(env_table, title="[header] Runtime Environment [/header]",
                        border_style="magenta", padding=(0, 1)))

    # ── 2. Services ──────────────────────────────────────────────────────────
    svc_table = Table(show_header=True, header_style="header", box=box.SIMPLE, padding=(0, 1))
    ...
    services = [
        ("Gateway",    "http", f"http://localhost:{GATEWAY_PORT}/health", None,  "Sensory + Armor"),
        ("ChromaDB",   "http", f"http://localhost:{CHROMA_PORT}/api/v1/heartbeat", None, "Semantic Memory"),
        ...
        ("Ollama",     "http", f"http://localhost:{OLLAMA_PORT}/api/tags", None, "Local LLM (optional)"),
        ("Registry",   "tcp",  "localhost",  REGISTRY_PORT, "Swarm Registry"),
    ]

    online_count = 0
    weights = {"Gateway": 30, "ChromaDB": 20, "PostgreSQL": 15, "Redis": 10, "Neo4j": 15, "ARIA UI": 10}
    ...
    for name, probe, target, port, role in services:
        ...
        if ok:
            online_count += 1
        status_cell = "[success]✓ Online[/success]" if ok else (
            "[muted]— Offline[/muted]" if name in ("Ollama", "Registry") else "[error]✗ Offline[/error]"
        )
        lat_cell = f"{lat}ms" if lat else "—"
        svc_table.add_row(name, status_cell, lat_cell, role)

    zfi = sum(weights.get(n, 0) for n, *_ in services
    ...
    if session_file.exists():
        ...
        try:
            ...
            cfg_table.add_row("Active Aeon", f"[accent]{active}[/accent]", "zana aeon use <id>")
        except Exception:
            cfg_table.add_row("Active Aeon", "[muted]— unreadable[/muted]", "")
    else:
        cfg_table.add_row("Active Aeon", "[muted]herald (default)[/muted]", "")

    console.print(Panel(cfg_table, title="[header] Configuration [/header]",
                        border_style="magenta", padding=(0, 1)))
    ...
    if zfi >= 90:
        ...
        console.print("[warning]⚠ System partially operational. Start missing services with: zana start[/warning]\n")
    else:
