...
def get_cloudflared_url():
    ...
    if system == "linux":
        ...
        if machine in ["x86_64", "amd64"]:
            return (
                base_url + "cloudflared-darwin-amd64.tgz"
            )  # Actually darwin binary is inside tgz, let's use homebrew suggestion or direct binary if available.
...
def install_cloudflared():
    ...
    if system == "darwin":
        console.print(
            "[muted]On macOS, please install cloudflared via Homebrew: brew install cloudflare/cloudflare/cloudflared[/muted]"
        )
        return False
    ...
    if not url:
        console.print(
            "[error]Unsupported OS or architecture for automatic cloudflared installation.[/error]"
        )
        return False
...
def cmd_expose(port: int = 54448):
    ...
    if system == "darwin":
        ...
        if shutil.which("cloudflared"):
            ...
            console.print(
                "[warning]cloudflared is not installed. Run: brew install cloudflare/cloudflare/cloudflared[/warning]"
            )
            return
    else:
    ...
    try:
        ...
        if url:
            ...
            console.print(
                f"[accent]Remote Access URL:[/accent] [bold white]{url}[/bold white]\n"
            )
            console.print(
                "[muted]Press Ctrl+C to collapse the bridge and revoke access.[/muted]"
            )
            ...
            console.print(
                "[error]Failed to establish the bridge. Check if cloudflared is running correctly.[/error]"
            )
            process.terminate()
