...
def _render_warrior_table(warriors: list[dict]) -> Panel:
    ...
    for i, w in enumerate(warriors, 1):
        ...
        fit_color = (
            "success" if fitness >= 0.8 else "warning" if fitness >= 0.5 else "muted"
        )
        ...
        dna = (
            w.get("dna_fingerprint", "")[:10] + "…" if w.get("dna_fingerprint") else "—"
        )
        table.add_row(
            str(i),
    ...
    fit_color = (
        "success" if avg_fit >= 0.8 else "warning" if avg_fit >= 0.5 else "muted"
    )
...
def cmd_swarm_init(warriors: int = 50, generations: int = 2000) -> None:
    ...
    with Progress(
        ...
        result = _gw_post(
            "/swarm/init",
            {"warriors": warriors, "generations": generations},
            timeout=30,
        )
        ...
        if "error" in result:
            ...
            console.print(
                "[muted]Gateway swarm endpoint not available. Bootstrapping locally…[/muted]"
            )
    ...
    console.print(
        f"[success]✓ Red Queen online. {spawned} warriors deployed.[/success]"
    )
    console.print(
        "[muted]Run [accent]zana swarm status[/accent] to watch evolution.[/muted]\n"
    )
...
def _bootstrap_local(warriors: int, generations: int, progress, task) -> None:
    ...
    _save_state(state)
    console.print(
        f"[success]✓ Red Queen bootstrapped locally. {warriors} warriors ready.[/success]"
    )
    console.print(
        "[muted]Run [accent]zana swarm status[/accent] to inspect the fleet.[/muted]\n"
    )
...
def cmd_swarm_status(watch: bool = False) -> None:
    def _fetch() -> Panel:
        ...
        if not warriors:
            return Panel(
                "[muted]No warriors deployed. Run: zana swarm init[/muted]",
                title="[header] RED QUEEN [/header]",
                border_style="magenta",
            )
...
def cmd_swarm_sync() -> None:
    ...
    if rules is None:
        console.print(
            "[error]Registry offline (port 54445). Start it via: zana start[/error]"
        )
        return
    ...
    console.print(
        "[muted]Rules pulled. LLMGuard validated. Assimilating into local engine…[/muted]"
    )
    ...
    if assimilation and "error" not in assimilation:
        ...
        console.print(
            "[muted]Gateway not available for assimilation. Rules stored locally.[/muted]\n"
        )
...
def cmd_swarm_query(fact_key: str) -> None:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "remote_query", root / "swarm" / "remote_query.py"
        )
        ...
        if result.found and result.rule:
            rule = result.rule
            console.print("[success]✓ Swarm rule found:[/success]")
            console.print(
                f"  [bold white]{rule.rule_data.get('name', rule.rule_id)}[/bold white]"
            )
            console.print(
                f"  [muted]from[/muted] [accent]{rule.creator_node}[/accent]  "
