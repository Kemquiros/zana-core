...
SERVICES: list[tuple[str, Literal["http", "tcp"], str, int | None, str]] = [
    ...
    ("ChromaDB",   "http", "http://localhost:58001/api/v1/heartbeat", None, "Semantic Memory"),
...
def _build_table(statuses: list[ServiceStatus], zfi: int) -> Panel:
    ...
    for s in statuses:
        status_cell = "[success]✓ Online[/success]" if s.online else "[error]✗ Offline[/error]"
    ...
    return Panel(table, title="[header] ZANA STATUS [/header]", subtitle=footer,
                 border_style="magenta", padding=(0, 1))


def _compute_zfi(statuses: list[ServiceStatus]) -> int:
    weights = {"Gateway": 30, "ChromaDB": 20, "PostgreSQL": 15,
               "Redis": 10, "Neo4j": 15, "ARIA UI": 10}
