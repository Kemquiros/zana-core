...
def _render_trace(trace: list[dict]) -> None:
    ...
    for step in trace:
        ...
        if confidence is not None:
            conf_color = "success" if confidence >= 0.8 else "warning" if confidence >= 0.5 else "error"
...
def cmd_reason(fact_raw: str, remote: bool = False) -> None:
    try:
        ...
        console.print("[muted]Usage: zana reason 'fact_key=value'  or  zana reason '{\"fact_key\": \"k\", \"value\": 0}'[/muted]")
        return
    ...
    try:
        ...
        console.print(f"[error]Gateway error {e.response.status_code}: {e.response.text[:200]}[/error]")
        return
    ...
    if deductions:
        ...
        for d in deductions:
            ...
            conf_color = "success" if conf >= 0.8 else "warning" if conf >= 0.5 else "error"
            table.add_row(
                d.get("conclusion", "—"),
                f"[{conf_color}]{conf:.2f}[/{conf_color}]",
                d.get("action", "—"),
            )
        console.print(Panel(table, title="[header] Deductions [/header]",
