...
class StrategyEvolution:
        ...
        raw_ideas = [
            {
                ...
                "traits": {
                    ...
                    "governance": random.choice(
                        ["John_Only", "DAO", "Meritocracy", "Autonomous"]
                    ),
    ...
    def mutate(self, idea: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if trait_to_mutate == "access":
            new_idea["traits"][trait_to_mutate] = random.choice(
                ["Open", "Closed", "Gated", "Hybrid"]
            )
            ...
            new_idea["traits"][trait_to_mutate] = random.choice(
                ["Subscription", "Donation", "Tokenomics", "Free", "Pay-per-Compute"]
            )
    ...
    def crossover(
        ...
        for key in parent_a["traits"]:
            child_traits[key] = random.choice(
                [parent_a["traits"][key], parent_b["traits"][key]]
            )
        return {
    ...
    def evaluate_fitness(self, idea: Dict[str, Any]) -> float:
        ...
        if t["platform"] == "Crypto/Web3":
            score += 20
        elif t["platform"] == "Cloud/SaaS":
            score += 15  # Good for John's revenue
        elif t["platform"] == "Biological/Swarm":
            score += 25  # High end-goal alignment

        # 3. Revenue: Tokenomics + Subscription crossover is strong
        if t["revenue"] == "Tokenomics":
            score += 20
        elif t["revenue"] == "Subscription":
            score += 15

        # 4. Governance: DAO/Autonomous supports the Hive Mind
        if t["governance"] == "DAO":
            score += 20
        elif t["governance"] == "Autonomous":
            score += 25
        elif t["governance"] == "John_Only":
            score += 10  # Initial control is good but not scalable

        # Balance Score
        return score + (random.random() * 5)

    def run_tournament(self):
        print("--- 🏆 ZANA TOURNAMENT OF IDEAS: THE STRATEGY GENESIS ---")
