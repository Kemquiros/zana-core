...
class AutonomousOrchestrator:
    ...
    def run_autonomous_step(
        ...
        if "log" in task_description.lower() or chosen_skill == "eml_reconstruct_log":
            outcome = self.math_agent.execute_math_task(
                "eml_reconstruct_log", x=np.random.uniform(1.1, 5.0)
            )
        else:
            outcome = self.math_agent.execute_math_task(
                "eml_reconstruct_exp", x=np.random.uniform(0.1, 2.0)
            )
...
if __name__ == "__main__":
    ...
    res = orchestrator.run_autonomous_step(
        mock_ctx, "Calculate fundamental log constant"
    )
