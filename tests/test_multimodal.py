...
def make_silent_wav(duration_s: float = 0.5, sr: int = 16000) -> bytes:
    ...
    header = struct.pack(
        ...
        16, 1, 1, sr, sr * 2, 2, 16, b"data", len(data)
    )
    return header + data
...
def test_perception_event() -> bool:
    ...
    evt2 = PerceptionEvent(
        ...
        vision_features=VisionFeatures(
            scene_type="screen", entities=["code", "terminal", "error"],
            emotional_context="stressed", text_content="", confidence=0.85,
        ),
    )
    ...
    evt3 = PerceptionEvent(modality="multimodal", text="audio + imagen", session_id="s1")
...
def test_audio_processor() -> bool:
    ...
    ok1 = (isinstance(features, AudioFeatures) and
           0.0 <= features.rms_energy <= 1.0 and
           features.zero_crossing_rate >= 0.0)
    note = f"dur={features.duration_s}s, rms={features.rms_energy:.3f}" if ok1 else "crash"
    print(f"  [{'✅' if ok1 else '❌'}] Features sin crash: {note}")
    if ok1: passed += 1

    # Caso 2: emotion inference
    loud_features = AudioFeatures(duration_s=1.0, rms_energy=0.7,
                                  zero_crossing_rate=0.15, dominant_freq_hz=200.0,
                                  speech_rate_wpm=200.0)
    emotion = proc.infer_emotion_from_features(loud_features)
    ok2 = emotion == "agitated"
    print(f"  [{'✅' if ok2 else '❌'}] Emoción alta energía = 'agitated' (got '{emotion}')")
    if ok2: passed += 1

    # rms=0.05 < 0.1 AND wpm=60 < 80 → "calm" (no llega a "subdued" que requiere rms < 0.05)
    calm_features = AudioFeatures(duration_s=1.0, rms_energy=0.05,
                                  zero_crossing_rate=0.02, dominant_freq_hz=150.0,
                                  speech_rate_wpm=60.0)
    emotion2 = proc.infer_emotion_from_features(calm_features)
    ok3 = emotion2 == "calm"
    print(f"  [{'✅' if ok3 else '❌'}] Emoción rms=0.05, wpm=60 = 'calm' (got '{emotion2}')")
    if ok3: passed += 1
...
def test_tts_engine() -> bool:
    ...
    print(f"  [{'✅' if ok1 else '❌'}] Silent fallback = {len(silent)} bytes, header RIFF")
    if ok1: passed += 1
...
def test_agentcard_multimodal() -> bool:
    ...
    print(f"  [{'✅' if file_part_ok else '❌'}] FilePart struct con mimeType declarado")
    if file_part_ok: passed += 1
