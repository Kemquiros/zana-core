...
def test_phase5_e2e():
    ...
    print(
        f"Backend: {plan1['routing']['backend']} | Model: {plan1['routing']['model']}"
    )
    ...
    print(
        f"Backend: {plan2['routing']['backend']} | Model: {plan2['routing']['model']}"
    )
    ...
    # 4. Validation
    print("\n--- 🧠 ANALYSIS ---")
    if plan2["resonance"]["surprise"] > plan1["resonance"]["surprise"] and plan3.get(
        "cache_hit"
    ):
    ...
    print(
        f"Phi Partitioning (Core Reasoning): {plan1['partitions']['core_reasoning']} tokens (61.8%)"
    )
