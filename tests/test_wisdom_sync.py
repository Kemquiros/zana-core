...
def test_dynamic_rule():
    ...
    resp = requests.post("http://localhost:54444/mcp/reasoning/add_rule", json={"rule": new_rule})
    print(f"Carga de regla: {resp.status_code}")

    # 3. Assert facts to trigger it
    requests.post("http://localhost:54444/mcp/reward", json={"user_reaction": 0.9, "comment": "Excellent"})
