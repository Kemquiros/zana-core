...
def test_llm_guard() -> bool:
    ...
    banned_rule = {
        "name": "Kill_Switch",
        "conditions": [{"fact_key": "active_peers", "operator": "Gte", "value": {"Number": 1}}],
    ...
    print(f"  [{'✅' if ok2 else '❌'}] Banned effect → DANGEROUS (got {r2.threat_level.value})")
    if ok2: passed += 1
    ...
    print(f"  [{'✅' if ok3 else '❌'}] Empty conditions → SUSPICIOUS (got {r3.threat_level.value})")
    if ok3: passed += 1

    # Caso 4: prompt injection en template → DANGEROUS
    inject_rule = {
        "name": "Look_Normal",
        "conditions": [{"fact_key": "mood", "operator": "Eq", "value": {"String": "happy"}}],
    ...
    print(f"  [{'✅' if ok4 else '❌'}] Prompt injection → DANGEROUS (got {r4.threat_level.value})")
    if ok4: passed += 1
...
def test_remote_query() -> bool:
    ...
    print(f"  [{'✅' if ok else '❌'}] Registry offline → graceful fallback (error='{result.error}')")

    # Verificar que _rule_covers_fact funciona correctamente
    mock_rule = {
        "rule_data": {
            "conditions": [{"fact_key": "machine_health_avg"}, {"fact_key": "liquidity"}]
        }
    }
...
def test_civic_ledger_logic() -> bool:
    ...
    def mirror(rule_json: str, operation: str, author: str) -> str:
        ...
        entry = {"rule_name": rule_name, "rule_hash": rule_hash,
    ...
    ok_chain = verify()
    print(f"  [{'✅' if ok_chain else '❌'}] Ledger chain integrity verified ({len(ledger)} entries)")
...
def test_mastery_map_logic() -> bool:
    ...
    XP_THRESHOLDS = {"Larva": 0, "Initiate": 100, "Warrior": 300, "Champion": 600, "Legend": 1000}
