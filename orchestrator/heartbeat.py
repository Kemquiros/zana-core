...
class AeonHeartbeat:
    ...
    def _parse_changelog(self, body: str) -> dict[str, list[str]]:
        ...
        sections = {"Core": [], "Memory": [], "Senses": [], "UI": [], "Other": []}
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if latest_v and latest_v != current_v:
                    logger.info(
                        f"¡Nueva evolución detectada! v{current_v} -> v{latest_v}"
                    )
                    ...
                    if (
                        staged_data.get("latest_version") == latest_v
                        and staged_data.get("status") == "NOTIFIED"
                    ):
                        return
    ...
    async def start(self):
        ...
        logger.info(
            f"Servicio de Latido iniciado. Intervalo: {self.interval / 60} min."
        )
