...
def recall(state: AgentState):
    ...
    retrieved_context = "[MOCK MEMORY]: VECANOVA focuses on AI agents and 'Empire of One'."
...
def planner(state: AgentState):
    ...
    if not ANTHROPIC_API_KEY:
        ...
        return {"plan": ["1. Research target audience", "2. Design glassmorphism hero", "3. Implement async capture"], "next_node": "executor"}
...
def executor(state: AgentState):
    ...
    return {"messages": state["messages"] + [AIMessage(content="[EXECUTOR]: Task executed according to plan.")], "next_node": "critic"}
...
if __name__ == "__main__":
    import sys
    query = sys.argv[1] if len(sys.argv) > 1 else "Build a luxury landing page plan for VECANOVA"
    run_task(query)
