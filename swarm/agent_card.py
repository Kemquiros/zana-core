...
def build_agent_card(node_id: str, dna: Any | None = None) -> dict:
    ...
    base_url = os.getenv(
        "ZANA_PUBLIC_URL", f"http://localhost:{os.getenv('ZANA_GATEWAY_PORT', '54446')}"
    )
                ...
                "examples": [
                    {
                        ...
                        "output": "{response_text, emotion, kalman_surprise}",
                    }
                ],
    ...
    if dna is not None:
        try:
            fp = (
                dna.get_fingerprint()
                if hasattr(dna, "get_fingerprint")
                else hashlib.sha256(str(dna).encode()).hexdigest()
            )
