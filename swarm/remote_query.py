...
class RemoteQuery:
    ...
    def query_swarm(self, fact_key: str, context: dict | None = None) -> QueryResult:
        ...
        """
        print(
            f"📡 [REMOTE QUERY] Node '{self.node_id}' asking swarm about fact: '{fact_key}'"
        )
        try:
            resp = requests.get(
            ...
            if resp.status_code != 200:
                return QueryResult(
                    found=False, error=f"Registry returned {resp.status_code}"
                )
            ...
            print(
                f"   ↳ Best candidate: '{result.rule_data.get('name', '?')}' "
            ...
            print("   ↳ Registry offline. Continuing with local reasoning only.")
