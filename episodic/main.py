...
async def create_episode(episode: EpisodeCreate):
    ...
    with psycopg.connect(CONN_STR) as conn:
        with conn.cursor() as cur:
            ...
            """, (
                episode.session_id, episode.event_type, episode.subject, 
                psycopg.types.json.Jsonb(episode.context), episode.outcome, 
                episode.outcome_type, episode.tags, episode.project, embedding_list
            ))
            res = cur.fetchone()
            if not res:
