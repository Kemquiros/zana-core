...
class CognitiveKalmanFilter:
    ...
    def __init__(
        ...
        self.dim = dim
        self.inner = zana_steel_core.PyKalmanFilter(
            dim, process_noise, measurement_noise
        )
    ...
    def update(self, observation: np.ndarray) -> float:
        ...
        if observation.shape[0] != self.dim:
            raise ValueError(
                f"Observation dimension {observation.shape[0]} does not match state dimension {self.dim}"
            )
