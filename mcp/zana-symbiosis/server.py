...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        print(f"🔗 [MCP SOCKET] KoruOS Client (Aeon) connected.")

    def disconnect(self, websocket: WebSocket):
        self.active_connections.remove(websocket)
        print(f"🔗 [MCP SOCKET] KoruOS Client (Aeon) disconnected.")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if res.status_code == 200:
                episodes = res.json()
                context_str = "\n".join([f"- {ep.get('subject', '')}: {ep.get('outcome', '')}" for ep in episodes])
                return {"status": "success", "context": context_str, "source": "episodic_memory"}
