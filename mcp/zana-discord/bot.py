...
async def _reason(fact_raw: str) -> str:
    ...
    try:
        ...
        for d in deductions:
            lines.append(
                f"• **{d['rule']}** ({d.get('confidence', 0):.0%}) → {d['conclusion']}"
            )
            if d.get("action"):
...
@client.tree.command(
    name="sense", description="Send a message to ZANA's cognitive pipeline"
)
...
@client.tree.command(
    name="reason", description="Trigger symbolic reasoning with a structured fact"
)
...
async def on_ready():
    logger.info(
        "✓ ZANA Discord Bot ready — logged in as %s (ID: %s)",
        client.user.name,
        client.user.id,
    )
    await client.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.watching, name="the cognitive horizon"
        )
    )
...
async def on_message(message: discord.Message):
    ...
    async with message.channel.typing():
        session_id = (
            ...
            if is_dm
            else f"discord-{message.channel.id}"
        )
        response = await _sense(text, session_id)
