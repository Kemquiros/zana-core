...
def query_world_model(cypher_query: str) -> str:
    ...
    """
    if "CREATE" in cypher_query.upper() or "MERGE" in cypher_query.upper() or "DELETE" in cypher_query.upper():
        ...
        with driver.session() as session:
            ...
            for r in records:
                ...
                for i in range(len(rels)):
                    path_str += f" -[{rels[i]}]-> ({nodes[i]['id']} :{nodes[i]['labels'][0]})"
...
def get_session_state(session_id: str = "default") -> str:
    ...
    try:
        ...
        if state:
            return state
        return json.dumps({"active_project": None, "current_task": None, "hot_entities": []})
    except Exception as e:
...
def latent_sync_world(session_id: str = "default") -> str:
    ...
    try:
        ...
        with driver.session() as session:
            ...
            """
            result = session.run(cypher, project_id=active_project, entities=hot_entities)
