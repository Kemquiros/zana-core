...
def calculate_session_surprise(embedding_values: List[float]) -> str:
    ...
    try:
        ...
        return json.dumps(
            {"surprise": surprise, "uncertainty": uncertainty, "status": status}
        )
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{EPISODIC_API_URL}/episodes", json=payload, timeout=10.0
            )
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(
                f"{EPISODIC_API_URL}/episodes/similar", params=params, timeout=10.0
            )
            response.raise_for_status()
