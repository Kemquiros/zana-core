...
def _slack_post(endpoint: str, payload: dict) -> dict:
    with httpx.Client(timeout=_TIMEOUT) as c:
        r = c.post(f"https://slack.com/api/{endpoint}",
                   json=payload, headers=_slack_headers())
        return r.json()


def _slack_get(endpoint: str, params: dict | None = None) -> dict:
    with httpx.Client(timeout=_TIMEOUT) as c:
        r = c.get(f"https://slack.com/api/{endpoint}",
                  params=params, headers=_slack_headers())
        return r.json()
...
def slack_reason(fact: str, channel: str = "", remote: bool = False) -> str:
    ...
    try:
        ...
        parsed_v = v
    try:
        result = _gw_post("/reason", {"fact": {"fact_key": k.strip(), "value": parsed_v},
                                       "remote_query": remote})
...
def slack_status(channel: str = "") -> str:
    ...
    try:
        ...
        icon_map = {"stt": "🎤", "tts": "🔊", "llm": "🧠", "vision": "👁️", "kalman": "📐", "armor": "🛡️"}
...
def slack_channels() -> str:
    ...
    if not SLACK_BOT_TOKEN:
        return "error: SLACK_BOT_TOKEN not set"
    resp = _slack_get("conversations.list", {"limit": 200, "types": "public_channel,private_channel"})
    if not resp.get("ok"):
