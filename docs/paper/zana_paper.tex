...
\begin{tikzpicture}[
  ...
  \node[human]  (human)  at (-5.5, 0) {Human\\Operator\\[6pt]\scriptsize\normalfont (Goals,\\Intent,\\Attention)};
  \node[cortex] (cortex) at (0, 0)  {};
  \node[world]  (world)  at (5.5, 0){Digital\\World\\[6pt]\scriptsize\normalfont (APIs,\\Files,\\Sensors)};
  ...
  \draw[doublearr, color=xblue!80] (human.east) --
    node[below, font=\scriptsize\sffamily, color=gray!80!black, yshift=-2pt]{\textbf{intent} / context} (cortex.west);
  \draw[doublearr, color=xjade!90] (cortex.east) --
    node[below, font=\scriptsize\sffamily, color=gray!80!black, yshift=-2pt]{\textbf{actions} / responses} (world.west);
  \draw[arr, color=xblue!60, dashed, line width=1.2pt] (human.north east) to[bend left=45]
    node[right, font=\scriptsize\sffamily, color=gray!70!black, align=left]{\textbf{augmented}\\cognition} (human.south east);
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  eq/.style={font=\scriptsize\sffamily, color=gray!80, align=center, text width=3.5cm}
]
  \node[box] (prior) at (0,0)    {\textcolor{xblue}{Prior State}\\[3pt] $\hat{x}_{k|k-1},\;P_{k|k-1}$};
  \node[box] (gain)  at (5,0)    {\textcolor{xjade}{Kalman Gain}\\[3pt] $K_k$};
  \node[box] (post)  at (10,0)   {\textcolor{xamethyst}{Posterior}\\[3pt] $\hat{x}_{k|k},\;P_{k|k}$};
  \node[obs] (obs)   at (5,-2.5) {\textcolor{xrust}{Observation}\\[3pt] $z_k$};
  ...
  % Equation panel
  \node[eq, below=2.8cm of prior, text width=4cm, align=center]
    {$P_{k|k-1} = P_{k-1} + Q$};
  \node[eq, below=2.8cm of gain, text width=4cm, align=center]
    {$K_k = \dfrac{P_{k|k-1}}{P_{k|k-1}+R}$};
  \node[eq, below=2.8cm of post, text width=4cm, align=center]
...
\begin{tikzpicture}[
  ...
  note/.style={font=\scriptsize\sffamily, color=gray!80, align=center, text width=2.8cm}
]
  \node[box]   (seed)  at (0, 0)    {\textcolor{xamethyst}{Seed Skills}\\[3pt] (JSON)};
  \node[box]   (pop)   at (4.5, 0)    {\textcolor{xamethyst}{Population}\\[3pt] $N=50$ Warriors};
  \node[box]   (vm)    at (9, 0)    {\textcolor{xamethyst}{CognitiveVM}\\[3pt] Execution};
  \node[box]   (fit)   at (9,-2.5)  {\textcolor{xamethyst}{Fitness}\\[3pt] Evaluation};
  \node[box]   (mut)   at (4.5,-2.5)  {\textcolor{xblue}{Mutation} \&\\[3pt] \textcolor{xrust}{Selection}};
