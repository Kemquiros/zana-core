#!/usr/bin/env python3
"""
XANA Router v2 — Multi-Model Token-Optimized Router
=====================================================
...
Usage:
  python3 xana_router.py "query"
  python3 xana_router.py -r -v "query"          # classify only, verbose
  python3 xana_router.py -j "query"             # JSON output
  python3 xana_router.py --force claude "query" # force backend
  python3 xana_router.py --model opus "query"   # force Claude model
...
# ─── Load .env from xana-core root ────────────────────────────────────────────

def _load_env() -> None:
    """Load .env from xana-core directory into os.environ (only missing keys)."""
...
def _strip_gemini_noise(text: str) -> str:
    ...
    clean = [
        ...
        if line.strip() and not any(line.strip().startswith(p) for p in _GEMINI_LINE_NOISE)
    ]
    result = "\n".join(clean).strip()
...
def run_notebooklm(query: str, notebook_id: str | None = None) -> str:
    ...
    try:
        result = subprocess.run(
            ...
            timeout=120, cwd=str(NOTEBOOKLM_SKILL_DIR),
        )
        return result.stdout.strip() or result.stderr.strip() or "[NotebookLM] No output."
...
def run_gemini(query: str) -> str:
    ...
    try:
        ...
        if not output and ("not found" in result.stderr.lower() or "ModelNotFound" in result.stderr):
...
def run_pi_claude(query: str, model: ClaudeModel, tools: str = "read,bash,edit,write") -> str:
    """
    ...
    cmd = [
        PI_BIN,
        "--provider", PI_ANTHROPIC_PROVIDER,
        "--model", model_id,
        "--print",
        "--no-session",
        "--mode", "text",
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True,
            timeout=180, env={**os.environ, "ANTHROPIC_API_KEY": api_key},
        )
        return result.stdout.strip() or result.stderr.strip() or f"[Pi:{model.name}] No output."
    except subprocess.TimeoutExpired:
...
def run_pi_gemini(query: str) -> str:
    ...
    try:
        ...
        return result.stdout.strip() or result.stderr.strip() or "[Pi:Gemini] No output."
    except subprocess.TimeoutExpired:
...
def route(
    ...
    if verbose:
        ...
        print(
            f"[XANA Router] → {backend.value}{model_str}  "
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="xana_router",
        description="XANA Router v2 — Multi-model token-optimized routing.",
    )
    p.add_argument("query", nargs="?", help="Query (reads stdin if omitted)")
    p.add_argument("-r", "--route-only", action="store_true",
                   help="Classify only, do not execute")
    p.add_argument("-n", "--notebook-id", metavar="ID",
                   help="NotebookLM notebook ID")
    p.add_argument("--force", metavar="ROUTE",
    ...
    p.add_argument("--no-pi", action="store_true",
                   help="Disable Pi executor (emit signal instead)")
    p.add_argument("-v", "--verbose", action="store_true",
                   help="Print routing decision to stderr")
...
def main() -> None:
    ...
    if args.json:
        ...
        print(f"→ {out['route']}{model_str}  (backend:{out['reason']} | model:{out['model_reason']})")
    elif out["result"]:
        print(out["result"])
