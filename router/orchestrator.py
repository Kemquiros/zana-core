...
def pattern_pipeline(steps: list[str], initial_context: str = "") -> list[AgentResult]:
    ...
    for i, query_template in enumerate(steps):
        query = (
            query_template.format(context=context)
            if "{context}" in query_template
            else query_template
        )
...
def pattern_retrieve_then_reason(
    ...
    # Step 1: Retrieve
    retrieval = run_agent(
        retrieval_query, force_backend="notebooklm", notebook_id=notebook_id
    )
    if retrieval.error:
        return [retrieval]
    ...
    reasoning = run_agent(
        reasoning_query, force_backend="claude", force_model=claude_model
    )
...
def pattern_gemini_then_claude(
    ...
    synthesis = run_agent(
        synthesis_query, force_backend="claude", force_model=claude_model
    )
...
def format_results(results: list[AgentResult], fmt: str = "text") -> str:
    ...
    if len(results) > 1:
        ...
        lines.append(f"\n{'═' * 60}")
        lines.append(
            f"[SYNTHESIS] {len(non_error)}/{len(results)} agents responded successfully."
        )
        if non_error:
            lines.append(
                f"Fastest: {non_error[0].agent} ({non_error[0].latency_ms:.0f}ms)"
            )
...
def main() -> None:
    ...
    if pattern == "auto":
        ...
        if args.verbose:
            print(
                f"[Orchestrator] Fan-out to {len(agents)} agents: {agents}",
                file=sys.stderr,
            )
