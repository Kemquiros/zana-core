...
class ResonanceOrchestrator:
    ...
    def route_resonance(
        ...
        if context_segments and context_embeddings:
            compressed_segments = self.compressor.filter_segments(
                context_segments, context_embeddings
            )
            print(
                f"Context Filtered: {len(context_segments)} -> {len(compressed_segments)} segments."
            )
