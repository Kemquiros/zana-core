...
check_dependencies() {
    ...
    if ! command -v docker &>/dev/null; then
        ...
        echo -e "${CYAN}ℹ Install Docker from https://docs.docker.com/get-docker/ to unlock the full stack.${RESET}"
        echo -e "${CYAN}ℹ ZANA runs in SQLite-only mode (no Docker required) — perfect to start.${RESET}"
    fi
